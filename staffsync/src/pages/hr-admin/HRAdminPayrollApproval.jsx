...
export default function HRAdminPayrollApproval() {
  ...
  const pendingRuns = runs.filter((r) => r.status === 'pending_hr')
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-text-primary">Payroll Approval (HR)</h1>
        <p className="text-text-muted text-sm mt-1">Final HR sign-off before Finance can send payslips</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-text-muted">No payroll runs awaiting HR approval.</p>
        </div>
      ) : (
          ...
          {pendingRuns.map((run) => (
            <div key={run.id} className="glass rounded-2xl border border-amber-400/20 overflow-hidden">
              <div className="flex items-center gap-2 px-5 py-3 border-b border-white/5" style={{ background: 'rgba(245,158,11,0.06)' }}>
                ...
                <span className="text-sm font-semibold text-amber-400">Awaiting HR Approval</span>
                <span className="text-xs text-text-muted ml-1">(Manager already approved)</span>
              </div>
              <div className="p-5 space-y-4">
                <div>
                  <p className="text-lg font-bold text-text-primary">{run.period_label}</p>
                  <p className="text-xs text-text-muted">{run.period_start} → {run.period_end} · {run.employee_count} employees</p>
                </div>
                ...
                {rejectingId === run.id && (
                  <div className="space-y-2">
                    <textarea rows={2} className="w-full bg-surface-3 border border-border-default rounded-xl px-3 py-2 text-sm text-text-primary resize-none"
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => reject.mutate({ runId: run.id, reason })}
                ...
                {rejectingId !== run.id && (
                  <div className="flex gap-3">
                    ...
                    <button onClick={() => setRejectingId(run.id)}
      ...
      {runs.filter((r) => !['pending_hr'].includes(r.status)).length > 0 && (
        <div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5">
                  {['Period','Status','Net Pay'].map((h) => (
                    <th key={h} className="py-3 px-4 text-left text-[10px] font-semibold text-text-muted uppercase">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {runs.filter((r) => r.status !== 'pending_hr').map((r) => (
                  <tr key={r.id} className="hover:bg-white/2">
                    <td className="py-3 px-4 font-medium text-text-primary">{r.period_label}</td>
                    <td className="py-3 px-4">
                      <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold"
                        style={{ color: r.status==='paid'?'#34d399':r.status==='rejected'?'#f87171':'#94a3b8', background: r.status==='paid'?'rgba(16,185,129,0.15)':r.status==='rejected'?'rgba(239,68,68,0.15)':'rgba(71,85,105,0.15)' }}>
